...
const deploy = async () => {
  ...
  penpieSYContract = await ethers.getContractAt("PenpieSY", penpieSY.address);
};
...
const previewRewards = async (account: string, harvest : boolean) => {
  if(harvest)
    await pendleStakingContract.harvestMarketReward("0x7D49E5Adc0EAAD9C027857767638613253eF125f");
  const pendingRewards = await masterPenpieContract.pendingTokens(mPendleContract.address, account, pendleContract.address);
  console.log("Pending PENDLE reward:", pendingRewards.pendingPenpie);
  console.log("Pending PNP reward:", pendingRewards.pendingBonusToken);
  console.log("PENDLE balance:", await pendleContract.balanceOf(account));
  console.log("PNP balance:", await PNPContract.balanceOf(account));
  
}

const moveTime = async (secondsToMove:number)=> {
  await network.provider.send("evm_increaseTime", [secondsToMove]);
  await network.provider.send("evm_mine", []);
  console.log("move Time: ",[secondsToMove],"(seconds)");
}
...
async function main() {
  ...
  // Transferring all 3 tokens, viz Pendle, mPendle and mPendleReciept to myWallet
  ...
  console.log("pendle-----------");
  await previewRewards(penpieSYContract.address,false);
  await depositSY(myWallet, pendleContract, ethers.utils.parseEther("10000"), false);
  await moveTime(3600*24);
  await previewRewards(penpieSYContract.address,true);
  await depositSY(myWallet, pendleContract, ethers.utils.parseEther("10000"), false);
  await previewRewards(penpieSYContract.address,false);

  console.log("mPendle-----------");
  await previewRewards(penpieSYContract.address,false);
  await depositSY(myWallet, mPendleContract, ethers.utils.parseEther("10000"), false);
  await moveTime(3600*24);
  await previewRewards(penpieSYContract.address,true);
  await depositSY(myWallet, mPendleContract, ethers.utils.parseEther("10000"), false);
  await previewRewards(penpieSYContract.address,false);

  console.log("mPendleReciept-----------");
  await previewRewards(penpieSYContract.address,false);
  await depositSY(myWallet, mPendleRecieptContract, ethers.utils.parseEther("10000"), false);
  await moveTime(3600*24);
  await previewRewards(penpieSYContract.address,true);
  await depositSY(myWallet, mPendleRecieptContract, ethers.utils.parseEther("10000"), false);
  await previewRewards(penpieSYContract.address,false);
  ...
  console.log("pendle-----------");
  await previewRewards(penpieSYContract.address,false);
  await redeemSY(myWallet, pendleContract, ethers.utils.parseEther("1000"));
  await moveTime(3600*24);
  await previewRewards(penpieSYContract.address,true);
  await redeemSY(myWallet, pendleContract, ethers.utils.parseEther("1000"));
  await previewRewards(penpieSYContract.address,false);
  ...
  console.log("mPendle-----------");
  await previewRewards(penpieSYContract.address,false);
  await redeemSY(myWallet, mPendleContract, ethers.utils.parseEther("1000"));
  await moveTime(3600*24);
  await previewRewards(penpieSYContract.address,true);
  await redeemSY(myWallet, mPendleContract, ethers.utils.parseEther("1000"));
  await previewRewards(penpieSYContract.address,false);
  ...
  console.log("mPendleReciept-----------");
  await previewRewards(penpieSYContract.address,false);
  await redeemSY(myWallet, mPendleRecieptContract, ethers.utils.parseEther("1000"));
  await moveTime(3600*24);
  await previewRewards(penpieSYContract.address,true);
  await redeemSY(myWallet, mPendleRecieptContract, ethers.utils.parseEther("1000"));
  await previewRewards(penpieSYContract.address,false);
