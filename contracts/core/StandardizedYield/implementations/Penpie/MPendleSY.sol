...
import "./ISmartPendleConvertor.sol";
import "./IMasterPenpie.sol";
import "./IStableSwapRouter.sol";
import "./IPancakeStableSwapTwoPool.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract MPendleSY is SYBaseWithRewards {
    ...
    ISmartPendleConvertor public smartPendleConvertor;
    ...
    constructor (
        ...
        address _mPendlePenpieRewarder
    )
        SYBaseWithRewards(_name, _symbol, _mPendle)
    {
        Pendle = _Pendle;
        mPendle = _mPendle;
        PNP = _PNP;
        smartPendleConvertor = ISmartPendleConvertor(_smartPendleConvertor);
        ...
        _safeApproveInf(Pendle, _smartPendleConvertor);
    }    
    ...
    function _deposit(
        ...
        amountSharesOut = _calcSharesOut(mPendleReceiptReceived, totalSupply(), getTotalAssetOwned() - mPendleReceiptReceived);
    }
    ...
    function previewPendleTomPendleConvert(
        ...
        if (mPendleToPendle < smartPendleConvertor.buybackThreshold()) {
            ...
            convertRatio = (convertAmount * 10000) / amountPendleToConvert;
        }
    ...
    function _previewRedeem(
        ...
        if (tokenOut == mPendle || tokenOut == mPendleReceiptToken) {
            ...
            tokenPath[1] = Pendle;
            poolPath[0] = pendleMPendlePool;
            (uint256 pendleOut) = IPancakeStableSwapTwoPool(pendleMPendlePool).get_dy(1, 0, amountSharesToRedeem);
